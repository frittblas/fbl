...
void CameraSystem::Init(Coordinator& ecs) {
	...
	for (auto const& entity : mEntities)
	{
		...
		// find x,y,w,h
		int x = (Game::DeviceResW / 2) - Game::TileSize * cam.deadZoneW;
		int y = (Game::DeviceResH / 2) - Game::TileSize * cam.deadZoneH;
		...
		int id = fbl_create_prim(FBL_NORMAL_RECT, x, y, w, h, 0, false, false); // create debug rect
...
void CameraSystem::Update(Coordinator& ecs) {
	...
	for (auto const& entity : mEntities)
	{
		...
		//cam.x = pos.x - Game::DeviceResW / 2;
		//cam.y = pos.y - Game::DeviceResH / 2;

		// lerped camera using the above _lerp function
		//cam.x = _lerp(pos.x - Game::DeviceResW / 2, cam.x, 0.05);
		//cam.y = _lerp(pos.y - Game::DeviceResH / 2, cam.y, 0.05);

		// lerp formula
		//vActual = vDesired * factor + vActual * (1.0 - factor);

		// "inline" lerp
		//cam.x = (pos.x - Game::DeviceResW / 2) * 0.05 + cam.x * (1.0 - 0.05);
		//cam.y = (pos.y - Game::DeviceResH / 2) * 0.05 + cam.y * (1.0 - 0.05);

		// camera trap (or deadzone) + lerp (subtract and add 1 pixel att the end bc otherwise always true)
		if (pos.x < ((Game::DeviceResW / 2) - Game::TileSize * cam.deadZoneW) + fbl_get_camera_x() - 1) {
			cam.x = ((pos.x - Game::DeviceResW / 2) + Game::TileSize * cam.deadZoneW) * cam.damp + cam.x * (1.0 - cam.damp);
		...
		if (pos.y < ((Game::DeviceResH / 2) - Game::TileSize * cam.deadZoneH) + fbl_get_camera_y() - 1) {
			cam.y = ((pos.y - Game::DeviceResH / 2) + Game::TileSize * cam.deadZoneH) * cam.damp + cam.y * (1.0 - cam.damp);
		...
		if (cam.x < 0) cam.x = 0;
		if (cam.y < 0) cam.y = 0;
		if (cam.x > Game::LogicalResW - (Game::DeviceResW - Game::LogicalResW)) cam.x = Game::LogicalResW - (Game::DeviceResW - Game::LogicalResW);
		if (cam.y > Game::LogicalResH - (Game::DeviceResH - Game::LogicalResH)) cam.y = Game::LogicalResH - (Game::DeviceResH - Game::LogicalResH);
