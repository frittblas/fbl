...
/* Walk the list but with parent object */
int DLWalkParent(DLLIST* List,
    int(*Func)(int, void*, void*),
    void* Args);

/* partition list (for DLQuickSort()) */
DLLIST *DLPartition(DLLIST **Start,
                    DLLIST **End,
                    int(*Func)(void *, void *));

/* Recursive quicksort, pass a compare function with a suitable key */
void DLQuickSort(DLLIST **List,
                 DLLIST **Start,
                 DLLIST **End,
                 int(*Func)(void *, void *));
