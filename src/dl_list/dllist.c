 ...
 *
 *
 *  Expanded to include DLPartition() and DLQuickSort()
 *  by Hans Strömquist 2015
 *
 */
...
DLLIST *DLPartition(DLLIST **Start, DLLIST **End, int(*Func)(void *, void *))
{
    ...
    while(Curr != NULL && Curr != *End)
    {
        ...
        if((*Func)(Curr->Object, Piv->Object))
        {
            
			/* swap contents of the Object */

            tmp = Small->Object;
            Small->Object = Curr->Object;
            Curr->Object = tmp;

			/* also swap the Tag in the parent NOTE: make this optional? */
    ...
    tmp = Piv->Object;
    Piv->Object = Small->Object;
    Small->Object = tmp;


	/* again, also swap the Tag in the parent NOTE: make this optional? */
...
void DLQuickSort(DLLIST **List, DLLIST **Start, DLLIST **End, int(*Func)(void *, void *))
{
    
    DLLIST *p;
    
    p = DLPartition(Start, End, Func);
    
    if(*Start != p)
        DLQuickSort(List, Start, &(p->Prev), Func);
    if(p != *End)
        DLQuickSort(List, &(p->Next), End, Func);
