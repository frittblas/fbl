...
int fbl_lua_init(const char *file)
{
	...
	strcat(new_path, "\0");
	luaL_loadfile(fbl_lua_env, new_path);
#else
	if (luaL_loadfile(fbl_lua_env, file)) {
		fprintf(FBL_ERROR_OUT, "%s", lua_tostring(fbl_lua_env, -1));	// print error msg, including syntax errors, very helpful!
	}
/*	if (luaL_loadfile(fbl_lua_env, "assets/main.lua") || lua_pcall(fbl_lua_env, 0, 0, 0))
	{
		fprintf(FBL_ERROR_OUT, "Cannot run main lua file: %s\n", lua_tostring(fbl_lua_env, -1));
		return -1;
	}
*/
#endif
...
/*
 *  Load a Lua file and run it. NOTE: get some error msg from this! (yes look above, fbl_lua_init() for a solution :))
 */
...
int lua_set_ui_elem_active(lua_State *lua_env)
{
	...
	int id = (int)lua_tonumber(lua_env, 1);
	int active = (int)lua_tonumber(lua_env, 1);
