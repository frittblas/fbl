...
int destroy_prim_phys(int tag, void *prim, void *dummy)
{
	...
	if(((FBL_PRIM *)prim)->physics_on)
	{
        ...
        if(((FBL_PRIM *)prim)->type != FBL_LINE) engine_phys_remove_body(((FBL_PRIM *)prim)->phys_body); // NOTE: why not FBL_LINE? check this (forgot)
...
int translate_phys_prim(int tag, void *prim, void *dummy)
{
	...
	if(((FBL_PRIM *)prim)->physics_on && ((FBL_PRIM *)prim)->active && (((FBL_PRIM *)prim)->type != FBL_LINE))
	{

		/* first get the physics body's position and angle */

		pos = cpBodyGetPosition(((FBL_PRIM *)prim)->phys_body);
		angle = cpBodyGetAngle(((FBL_PRIM *)prim)->phys_body);

		/* then set it on the prim */

		((FBL_PRIM *)prim)->dest_rect.x = (int)pos.x;
		((FBL_PRIM *)prim)->dest_rect.y = (int)pos.y;

		/* convert from radians to degrees */

		((FBL_PRIM *)prim)->angle = angle * 180.0 / M_PI;

		if(((FBL_PRIM *)prim)->type == FBL_TRI || ((FBL_PRIM *)prim)->type == FBL_RECT) translate_shape(((FBL_PRIM *)prim));
...
int render_prim(int tag, void *prim, void *dummy)
{
    ...
    if (((FBL_PRIM *)prim)->active)
    {

        if(((FBL_PRIM *)prim)->fix_to_screen)
        {
            temp_rect.x = ((FBL_PRIM *)prim)->dest_rect.x;
            temp_rect.y = ((FBL_PRIM *)prim)->dest_rect.y;
        }
        else
        {
            temp_rect.x = ((FBL_PRIM *)prim)->dest_rect.x - fbl_camera.x;
            temp_rect.y = ((FBL_PRIM *)prim)->dest_rect.y - fbl_camera.y;
        }

        temp_rect.w = ((FBL_PRIM *)prim)->dest_rect.w;
        temp_rect.h = ((FBL_PRIM *)prim)->dest_rect.h;
...

		{
			...
			SDL_SetRenderDrawColor(fbl_engine.renderer, ((FBL_PRIM*)prim)->color.r,
				((FBL_PRIM*)prim)->color.g,
				((FBL_PRIM*)prim)->color.b,
				((FBL_PRIM*)prim)->color.a);


			SDL_SetRenderDrawBlendMode(fbl_engine.renderer, SDL_BLENDMODE_BLEND);


			switch (((FBL_PRIM*)prim)->type)
			{

				case FBL_POINT:
					SDL_RenderDrawPoint(fbl_engine.renderer, temp_rect.x, temp_rect.y);
					if (((FBL_PRIM*)prim)->thick)
					{
				...
				case FBL_RECT:
					...
					else draw_shape((FBL_PRIM*)prim);
				break;
				case FBL_CIRCLE:
					if (((FBL_PRIM*)prim)->fill)
					{
						draw_circle_fill(temp_rect.x, temp_rect.y, ((FBL_PRIM*)prim)->radius);
					}
					else
					{
						draw_circle_(temp_rect.x, temp_rect.y, ((FBL_PRIM*)prim)->radius);
						if (((FBL_PRIM*)prim)->thick)
							draw_circle_(temp_rect.x, temp_rect.y, ((FBL_PRIM*)prim)->radius - 1);
					}
				break;
...
/*
 *  This is insanely slow! Only to be used for a couple of small balls or whatever. Will do better.
 */
void draw_circle_fill(int x, int y, int radius)
{
	...
	for (int w = 0; w < radius * 2; w++)
	{
		for (int h = 0; h < radius * 2; h++)
		{
			...
			{
				SDL_RenderDrawPoint(fbl_engine.renderer, x + dx, y + dy);
			}
		}
	}
