...
/*
 *	Give each sprite new x, y anf angle based on physics engine.
 */
int translate_phys_sprite(int tag, void *sprite, void *dummy)
{
	...
	if(((FBL_SPRITE *)sprite)->physics_on && ((FBL_SPRITE *)sprite)->active)
	{

		/* first get the physics body's position and angle */

		pos = cpBodyGetPosition(((FBL_SPRITE *)sprite)->phys_body);
		angle = cpBodyGetAngle(((FBL_SPRITE *)sprite)->phys_body);

		/* then set it on the sprite */

		((FBL_SPRITE *)sprite)->dest_rect.x = (int)pos.x;
		((FBL_SPRITE *)sprite)->dest_rect.y = (int)pos.y;

		/* convert from radians to degrees */

		((FBL_SPRITE *)sprite)->angle = angle * 180.0 / M_PI;
...
int animate_sprite(int tag, void *sprite, void *dummy)
{
	...
	if(((FBL_SPRITE *)sprite)->animated)
	{

		/* an animation speed of 7 will spend 7 frames on each image */

		if (fbl_engine.current_frame % ((FBL_SPRITE *)sprite)->animation_speed == 0)
		{

			/* move the source rect to the right to change image */

			((FBL_SPRITE *)sprite)->source_rect.x += ((FBL_SPRITE *)sprite)->source_rect.w;


			/* if on last frame reset xy and frame counter (if looped) */

			((FBL_SPRITE *)sprite)->current_frame++;
			if (((FBL_SPRITE *)sprite)->current_frame == ((FBL_SPRITE *)sprite)->animation_frames)
			{
				if (((FBL_SPRITE *)sprite)->anim_loop) {
					((FBL_SPRITE *)sprite)->current_frame = 0;
					((FBL_SPRITE *)sprite)->source_rect.x -= ((FBL_SPRITE *)sprite)->source_rect.w * ((FBL_SPRITE *)sprite)->animation_frames;
				}
				else {
					((FBL_SPRITE *)sprite)->source_rect.x -= ((FBL_SPRITE *)sprite)->source_rect.w;
					((FBL_SPRITE *)sprite)->animated = false;
				}
			}
...
int render_sprite(int tag, void *sprite, void *dummy)
{
	...
	if (((FBL_SPRITE*)sprite)->active)
	{

		if (((FBL_SPRITE*)sprite)->fix_to_screen)
		{
			temp_rect.x = ((FBL_SPRITE*)sprite)->dest_rect.x;
			temp_rect.y = ((FBL_SPRITE*)sprite)->dest_rect.y;
		}
		else
		{
			temp_rect.x = ((FBL_SPRITE*)sprite)->dest_rect.x - fbl_camera.x;
			temp_rect.y = ((FBL_SPRITE*)sprite)->dest_rect.y - fbl_camera.y;
		}
		...
		case FBL_SPRITE_ALIGN_UP_LEFT:
			//temp_rect.x = temp_rect.x - ((FBL_SPRITE *)sprite)->dest_rect.w / 2;
			//temp_rect.y = temp_rect.y - ((FBL_SPRITE *)sprite)->dest_rect.h / 2;
			break;
		...
		case FBL_SPRITE_ALIGN_DOWN_LEFT:
			//temp_rect.x = temp_rect.x - ((FBL_SPRITE *)sprite)->dest_rect.w / 2;
			temp_rect.y = temp_rect.y - ((FBL_SPRITE*)sprite)->dest_rect.h;
			break;
		}


		temp_rect.w = ((FBL_SPRITE*)sprite)->dest_rect.w;
		temp_rect.h = ((FBL_SPRITE*)sprite)->dest_rect.h;
...

		{
			...
			/* set blend modes and colors and alpha (benchmark this! update: switching blendmodes is kinda slow the rest very fast */
				...
				SDL_SetTextureBlendMode(fbl_texture, ((FBL_SPRITE*)sprite)->blendmode);
				SDL_SetTextureAlphaMod(fbl_texture, ((FBL_SPRITE*)sprite)->color.a);
				SDL_SetTextureColorMod(fbl_texture, ((FBL_SPRITE*)sprite)->color.r, ((FBL_SPRITE*)sprite)->color.g, ((FBL_SPRITE*)sprite)->color.b);




				SDL_RenderCopyEx(fbl_engine.renderer, fbl_texture, &((FBL_SPRITE*)sprite)->source_rect,
					&temp_rect, ((FBL_SPRITE*)sprite)->angle, NULL, ((FBL_SPRITE*)sprite)->flip);
